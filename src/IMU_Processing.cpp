...
static inline double stamp2sec(const builtin_interfaces::msg::Time &stamp) {
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  printf("[IMU] IMU Initializing: %.1f %%\n", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      imu_need_init = false;
      printf("[IMU] IMU Initials: Gravity: %.4f %.4f %.4f %.4f; acc covarience: "
               ...
               cov_gyr[2]);
      printf("[IMU] IMU Initials: ba covarience: %.8f %.8f %.8f; bg covarience: "
               "%.8f %.8f %.8f\n",
